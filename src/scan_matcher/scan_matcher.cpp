...
public:
  ...
  double minRange; //discard beams with reading less than this value 
...
static void process_laser(const frsm_planar_lidar_t * msg, void * user __attribute__((unused)))
{
  ...
  if (app->publish_pose) {
    ...
    // For debug:
    // frsm_pose_t_publish(app->lcm, "POSE_BODY_ALT", &pose);

    // Offset the internal estimate by the scan-to-body transform
...
int main(int argc, char *argv[])
{
  ...
  std::string full_param_file = std::string(getConfigPath()) +'/' + std::string(param_file);
  if (param_file.empty()) { // get param from lcm
    full_param_file = "";
  }

  if (full_param_file.empty()) {
    std::cout << "Get param from LCM\n";
    app->botparam = bot_param_get_global(app->lcm, 0);
  } else {
    std::cout << "Get param from file\n";
    app->botparam = bot_param_new_from_file(full_param_file.c_str());
  }
  if (app->botparam == NULL) {
    exit(1);
  }
